                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route("admin.categories.index") }}" method="GET" class="mb-6">
                        ...
                        <div class="mt-4 flex justify-end">
                            ...
                            <a href="{{ route("admin.categories.index") }}" class="btn-outline ml-2">Clear Filters</a>
                        </div>
                    </form>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Plants Count</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($categories as $category)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $category->plants_count }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <form action="{{ route("admin.categories.toggle-active", $category) }}" method="POST" class="inline">
                                                @csrf
                                                @method("PATCH")
                                                <button type="submit" class="btn-sm {{ $category->is_active ? "bg-green-500 text-white" : "bg-red-500 text-white" }} rounded-full">
                                                    {{ $category->is_active ? "Active" : "Inactive" }}
                                                </button>
                                            </form>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route("admin.categories.edit", $category) }}" class="text-primary-600 hover:text-primary-900 mr-3">Edit</a>
                                            <form action="{{ route("admin.categories.destroy", $category) }}" method="POST" class="inline">
                                                @csrf
                                                @method("DELETE")
                                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm("Are you sure you want to delete this category? This will also delete all associated plants.")">Delete</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
